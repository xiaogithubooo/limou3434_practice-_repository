...
// 绘制蛇
void PrintSnake(node* snake, int snakeLength, const int& red = -1, const int& green = -1, const int& blue = -1)
{
	// 某一段蛇节点的左上、右下坐标
	int left, top;
	int right, bottom;
...
// 绘制食物
void PaintFood(const node& food, const int& red = -1, const int& green = -1, const int& blue = -1) {
	// 食物的左上、右下坐标
	int left, top;
	int right, bottom;
...
void PrintMessages(const int& eatingTimes, const int& failTimes) {
	...
	// 输出游戏的名称
	drawtext(_T("Snake game"), &r, DT_CENTER | DT_SINGLELINE); // 后面两个宏是显示格式
