...
//			0.2.2.刷力扣的题目
//x				0.2.2.1.两个数组的交集：https://leetcode.cn/problems/intersection-of-two-arrays/
//x				0.2.2.2.搜索插入位置（二分法复习）：https://leetcode.cn/problems/search-insert-position/description/
//x				0.2.2.3.基于排列构建数组：https://leetcode.cn/problems/build-array-from-permutation/
//x				0.2.2.4.存在重复元素：https://leetcode.cn/problems/contains-duplicate/description/?envType=study-plan&id=shu-ju-jie-gou-ru-men&plan=data-structures&plan_progress=x5znyghm
//>		 0.3.写“顺序表”的博客
//x				0.3.1.leetcode题解
//x				0.3.2.顺序表详细博客
...
//> 2.5.搜索插入位置（二分法复习）
int FindIndexFunction(int* nums, int numsSize, int target)
{
    int begin = 0;
    int end = numsSize - 1;
    int index = 0;
...
int searchInsert_1(int* nums, int numsSize, int target)
{
    int flag = 0;
    int i = 0;
    if ((flag = FindIndexFunction(nums, numsSize, target)) != -1)
    {
        return flag;
    }
    else
...
//解法二：排序后查早重复的相邻元素
int fun(const void* a, const void* b)//没有超出时间限制
{
    return *((int*)a) - *((int*)b);
}
bool containsDuplicate(int* nums, int numsSize)
