...
//>		0.2.使用“无头无循环单向链表”思想
//			0.2.1.刷推荐和作业的题目
//x				0.2.1.1.移除链表元素：https://leetcode.cn/problems/remove-linked-list-elements/description/
//x				0.2.1.2.反转链表：https://leetcode.cn/problems/reverse-linked-list/
//x				0.2.1.3.链表的中间结点：https://leetcode.cn/problems/middle-of-the-linked-list/
//x				0.2.1.4.链表中倒数第k个结点：https://www.nowcoder.com/practice/529d3ae5a407492994ad2a246518148a?tpId=13&&tqId=11167&rp=2&ru=/activity/oj&qru=/ta/coding-interviews/question-ranking
//x				0.2.1.5.合并两个有序链表：https://leetcode.cn/problems/merge-two-sorted-lists/description/
...
struct ListNode* removeElements(struct ListNode* head, int val)
{
    struct ListNode* cur = head;//循环变量
    struct ListNode* newnode = NULL;//届时需要返回的新链表
    struct ListNode* tail = NULL;//记录cur的上一个节点

    while (cur)
    {
        if (cur->val != val)//1.如果不符合
        {
            if (newnode == NULL)//如果第一个节点为空，直接进行复制
            {
                newnode = tail = cur;
            }
            else//如果不是第一个节点了，就进行赋值
            {
                tail->next = cur;
                tail = cur;
            }
    ...
    if (newnode != NULL)//防止链表的每一个节点存储的数据都是val，出现解引用空指针的问题
        tail->next = NULL;
    return newnode;
}
...
struct ListNode* mergeTwoLists(struct ListNode* list1, struct ListNode* list2)
{
    ...
    while (list1 != NULL && list2 != NULL)//两者都不为空就走下去
    {
        if (list1->val >= list2->val)//第一个链表的元素比较大
        {
            if (newnode == NULL)
            {
                newnode = cur = list2;
    ...
    //将剩余的复制过来
    struct ListNode* list = ((list1 != NULL) ? list1 : list2);
    cur->next = list;
    return newnode;
}
